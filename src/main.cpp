...
int main() {
	...
	for (int i = 100; i <= MAT_SIZE; i += 100) {
		...
		// run product on the GPU @see cudaDGEMM
		cudaStatus = cudaDGEMM(MAT_SIZE, MAT_SIZE, MAT_SIZE, i, i, i, A_h, B_h, C_d);
		...
		if (cudaStatus != cudaSuccess) {
			fprintf(stderr, "cudaDGEMM failed!");
			return 1;
		}
